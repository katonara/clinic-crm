        ...
        <form method="POST" action="{{ route('admin.follow-ups.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="patient_id" required class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                    ...
                    <option value="{{ $p->id }}" {{ old('patient_id', $selectedPatientId) == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="booking_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                    ...
                    <option value="{{ $b->id }}" {{ old('booking_id', $selectedBookingId) == $b->id ? 'selected' : '' }}>{{ $b->booking_number }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="assigned_to" class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                    ...
                    <option value="{{ $s->id }}" {{ old('assigned_to') == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="follow_up_date" value="{{ old('follow_up_date', date('Y-m-d')) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                </div>
                <div>
                    ...
                    <select name="status" required class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                        ...
                        <option value="{{ $s }}" {{ old('status', 'not_contacted') === $s ? 'selected' : '' }}>{{ ucfirst(str_replace('_',' ',$s)) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div>
                ...
                <textarea name="notes" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg">{{ old('notes') }}</textarea>
            </div>
