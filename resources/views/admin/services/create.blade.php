    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <form method="POST" action="{{ route('admin.services.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg">{{ old('description') }}</textarea>
            </div>
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Price (RM) *</label>
                    <input type="number" step="0.01" name="price" value="{{ old('price') }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Duration (minutes) *</label>
                    <input type="number" name="duration_minutes" value="{{ old('duration_minutes', 30) }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                </div>
            </div>
            <div>
            ...
            <div>
                ...
                <div class="space-y-2 max-h-40 overflow-y-auto border border-gray-200 rounded-lg p-3">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" name="staff_ids[]" value="{{ $staff->id }}" class="rounded border-gray-300 text-blue-600">
                        <span class="ml-2 text-sm text-gray-700">{{ $staff->name }} ({{ ucfirst($staff->role) }})</span>
                    </label>
                    @endforeach
                    @if($staffMembers->isEmpty())<p class="text-sm text-gray-400">No staff available.</p>@endif
                </div>
            </div>
